...
import { ReturnRestart, createInitialState } from '@/ducks/restart/slice';
...
export const RestartEdit: FC<Props> = ({ type }) => {
  ...
  useEffect(() => {
    ...
    const st = new Date(restartData.start_time);
    const et = new Date(restartData.end_time);
    form.setValues({ ...restartData, start_time: st as any, end_time: et as any });
  }, [restartData]);
  ...
  const handleSubmit = async () => {
    setIsLoading(true);
    const isOK = await CustomConfirm(`記録票を編集しますか？\n利用者の方に確認を取ってください。`, '確認画面');
    if (!isOK) {
      setIsLoading(false);
      return;
    }
        ...
        <Box sx={{ display: 'flex', border: '1px solid' }}>
          ...
          <Box sx={{ display: 'flex', gap: 10, flexWrap: 'wrap', width: subContentWidth, padding: 5 }}>
            {CHECK_LIST.slice(0, 8).map((content) => (
              <Checkbox
                defaultChecked={isChecked(content.order)}
                key={content.order}
                value={content.order}
                label={content.name}
